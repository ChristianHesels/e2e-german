...
def handle_line(line, document_state, language, labels, stats):
  begin_document_match = re.match(conll.BEGIN_TUEBADZ_DOCUMENT_REGEX, line)
  if begin_document_match:
    document_state.assert_empty()
    document_state.doc_key = line.split(" ")[2]
    return None
    ...
    for n in ner.split("|"):
        handle_bit(word_index, n, document_state.ner_stack, document_state.ner)
...
def minimize_partition(name, language, labels, stats):
  input_path = name
  output_path = "data/training_data_09.json"
  count = 0
...
if __name__ == "__main__":
  ...
  minimize_partition("data/training_data_09.conll", "english", labels, stats)
